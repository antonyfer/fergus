<?php
namespace NoizzyCore\CPT\Shortcodes\AnimationHolder;
...
class AnimationHolder implements Lib\ShortcodeInterface {
	...
	function __construct() {
		$this->base = 'edge_animation_holder';
	...
	public function vcMap() {
		if ( function_exists( 'vc_map' ) ) {
			vc_map(
				array(
					'name'                    => esc_html__( 'Animation Holder', 'noizzy-core' ),
					'base'                    => $this->base,
	...
	public function render( $atts, $content = null ) {
		...
		$html = '<div class="edge-animation-holder ' . esc_attr( $animation ) . '" data-animation="' . esc_attr( $animation ) . '" data-animation-delay="' . esc_attr( $animation_delay ) . '"><div class="edge-animation-inner">' . do_shortcode( $content ) . '</div></div>';
