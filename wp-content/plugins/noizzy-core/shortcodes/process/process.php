<?php
namespace NoizzyCore\CPT\Shortcodes\Process;
...
class Process implements Lib\ShortcodeInterface {
	...
	public function vcMap() {
		if ( function_exists( 'vc_map' ) ) {
			vc_map(
				array(
					'name'            => esc_html__( 'Process', 'noizzy-core' ),
					'base'            => $this->base,
					...
					'params'          => array(
						...
						array(
							...
							'value'       => array(
								esc_html__( '2 Columns', 'noizzy-core' ) => 'two',
								esc_html__( '3 Columns', 'noizzy-core' ) => 'three',
								esc_html__( '4 Columns', 'noizzy-core' ) => 'four'
							),
							'save_always' => true
						),
						array(
							...
							'value'       => array(
								esc_html__( 'Default None', 'noizzy-core' ) => '',
	...
	public function render( $atts, $content = null ) {
		...
		$html = noizzy_core_get_shortcode_module_template_part( 'templates/process-template', 'process', '', $params );
	...
	private function getHolderClasses( $params, $args ) {
		...
		$holderClasses[] = ! empty( $params['number_of_columns'] ) ? 'edge-' . $params['number_of_columns'] . '-columns' : 'edge-' . $args['number_of_columns'] . '-columns';
		$holderClasses[] = ! empty( $params['switch_to_one_column'] ) ? 'edge-responsive-' . $params['switch_to_one_column'] : '';
