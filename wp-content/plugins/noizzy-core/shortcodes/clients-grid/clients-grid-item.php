<?php
namespace NoizzyCore\CPT\Shortcodes\ClientsGridItem;

use NoizzyCore\Lib;

class ClientsGridItem implements Lib\ShortcodeInterface {
	private $base;
	
	public function __construct() {
		$this->base = 'edge_clients_grid_item';
	...
	public function vcMap() {
		if ( function_exists( 'vc_map' ) ) {
			vc_map(
				array(
					'name'                    => esc_html__( 'Clients Grid Item', 'noizzy-core' ),
					'base'                    => $this->getBase(),
					'category'                => esc_html__( 'by NOIZZY', 'noizzy-core' ),
					'icon'                    => 'icon-wpb-clients-grid-item extended-custom-icon',
					'as_child'                => array( 'only' => 'edge_clients_grid' ),
					...
					'params'                  => array(
						...
						array(
							...
							'heading'     => esc_html__( 'Hover Image', 'noizzy-core' ),
							'description' => esc_html__( 'Select image from media library', 'noizzy-core' )
						),
						array(
							'type'        => 'textfield',
	...
	public function render( $atts, $content = null ) {
		...
		$html = noizzy_core_get_shortcode_module_template_part( 'templates/clients-grid-item', 'clients-grid', '', $params );
