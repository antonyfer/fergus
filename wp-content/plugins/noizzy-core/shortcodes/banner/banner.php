<?php
namespace NoizzyCore\CPT\Shortcodes\Banner;
...
class Banner implements Lib\ShortcodeInterface {
	...
	public function vcMap() {
		if ( function_exists( 'vc_map' ) ) {
			vc_map(
				array(
					'name'                      => esc_html__( 'Banner', 'noizzy-core' ),
					'base'                      => $this->getBase(),
					...
					'params'                    => array(
						...
						array(
							...
							'heading'    => esc_html__( 'Image Overlay Color', 'noizzy-core' )
						),
						array(
							'type'        => 'dropdown',
							'param_name'  => 'hover_behavior',
							'heading'     => esc_html__( 'Hover Behavior', 'noizzy-core' ),
							'value'       => array(
								esc_html__( 'Visible on Hover', 'noizzy-core' )   => 'edge-visible-on-hover',
								esc_html__( 'Visible on Default', 'noizzy-core' ) => 'edge-visible-on-default',
								esc_html__( 'Disabled', 'noizzy-core' )           => 'edge-disabled'
							),
							'save_always' => true
						),
						array(
							...
							'heading'     => esc_html__( 'Info Position', 'noizzy-core' ),
							'value'       => array(
								esc_html__( 'Default', 'noizzy-core' )  => 'default',
								esc_html__( 'Centered', 'noizzy-core' ) => 'centered'
							),
							'save_always' => true
						),
						array(
							...
							'heading'     => esc_html__( 'Info Content Padding', 'noizzy-core' ),
							'description' => esc_html__( 'Please insert padding in format top right bottom left', 'noizzy-core' )
						),
						array(
							'type'       => 'textfield',
							'param_name' => 'subtitle',
							'heading'    => esc_html__( 'Subtitle', 'noizzy-core' )
						),
						array(
							'type'        => 'dropdown',
							'param_name'  => 'subtitle_tag',
							'heading'     => esc_html__( 'Subtitle Tag', 'noizzy-core' ),
						...
						array(
							...
							'heading'    => esc_html__( 'Subtitle Color', 'noizzy-core' ),
							'dependency' => array( 'element' => 'subtitle', 'not_empty' => true )
						),
						array(
							'type'       => 'textfield',
						...
						array(
							...
							'heading'    => esc_html__( 'Link Text Color', 'noizzy-core' ),
							'dependency' => array( 'element' => 'link', 'not_empty' => true )
						),
						array(
							'type'       => 'textfield',
							'param_name' => 'link_top_margin',
							'heading'    => esc_html__( 'Link Text Top Margin (px)', 'noizzy-core' ),
							'dependency' => array( 'element' => 'link', 'not_empty' => true )
						)
					)
				)
	...
	public function render( $atts, $content = null ) {
		$args   = array(
			...
			'image'                => '',
			'overlay_color'        => '',
			'hover_behavior'       => 'edge-visible-on-hover',
		...
		$html = noizzy_core_get_shortcode_module_template_part( 'templates/banner', 'banner', '', $params );
	...
	private function getHolderClasses( $params, $args ) {
		...
		$holderClasses[] = ! empty( $params['info_position'] ) ? 'edge-banner-info-' . $params['info_position'] : 'edge-banner-info-' . $args['info_position'];
			...
			if ( ! empty( $title_light_words ) ) {
				foreach ( $light_words as $value ) {
					if ( ! empty( $split_title[ $value - 1 ] ) ) {
						$split_title[ $value - 1 ] = '<span class="edge-banner-title-light">' . $split_title[ $value - 1 ] . '</span>';
					}
				}
			}
	...
	private function getLinkStyles( $params ) {
		...
		if ( ! empty( $params['link_top_margin'] ) ) {
			$styles[] = 'margin-top: ' . noizzy_edge_filter_px( $params['link_top_margin'] ) . 'px';
		}
