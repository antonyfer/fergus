...
if ( $item_type === 'gallery' ) {
	$gallery = noizzy_core_get_portfolio_single_media();
	if ( $enable_count_images === 'yes' && is_array( $gallery ) && count( $gallery ) > 0 ) {
		...
		?>
		<div class="edge-gli-number-of-images-holder">
			<span><?php echo esc_attr( $counter ) . esc_html__( ' pics', 'noizzy-core' ); ?></span>
		</div>
	<?php }
}