...
namespace NoizzyCore\CPT\Shortcodes\Testimonials;
...
class Testimonials implements Lib\ShortcodeInterface {
				...
				array(
					...
					'params'                    => array(
						...
						array(
							'type'       => 'textfield',
							'param_name' => 'number',
							'heading'    => esc_html__( 'Number of Testimonials', 'noizzy-core' )
						),
						array(
							'type'        => 'autocomplete',
							'param_name'  => 'category',
							'heading'     => esc_html__( 'Category', 'noizzy-core' ),
							'description' => esc_html__( 'Enter one category slug (leave empty for showing all categories)', 'noizzy-core' )
						),
						array(
							'type'       => 'colorpicker',
							'param_name' => 'box_color',
							'heading'    => esc_html__( 'Content Box Color', 'noizzy-core' ),
						...
						array(
							...
							'value'       => array_flip( noizzy_edge_get_yes_no_select_array( false, true ) ),
							'save_always' => true,
                            'dependency'  => Array( 'element' => 'type', 'value' => array('boxed', 'standard') ),
							'group'       => esc_html__( 'Slider Settings', 'noizzy-core' )
						),
						array(
							'type'        => 'dropdown',
	...
	public function render( $atts, $content = null ) {
        ...
        return noizzy_core_get_cpt_shortcode_module_template_part( 'testimonials', 'testimonials', 'testimonials-' . $params['type'], '', $params );
	...
	private function getHolderClasses( $params ) {
		...
		$holderClasses[] = ! empty( $params['skin'] ) ? 'edge-testimonials-' . $params['skin'] : '';
	...
	private function getSliderData( $params ) {
		...
		$slider_data['data-slider-margin']          = in_array($params['type'], array('boxed')) ? 10 : '';
	...
	public function testimonialsCategoryAutocompleteSuggester( $query ) {
		...
		if ( is_array( $post_meta_infos ) && ! empty( $post_meta_infos ) ) {
			foreach ( $post_meta_infos as $value ) {
				...
				$data['label'] = ( ( strlen( $value['testimonials_category_title'] ) > 0 ) ? esc_html__( 'Category', 'noizzy-core' ) . ': ' . $value['testimonials_category_title'] : '' );
				$results[]     = $data;
			}
		}
		...
		if ( ! empty( $query ) ) {
			...
			if ( is_object( $testimonials_category ) ) {
				...
				if ( ! empty( $testimonials_category_title ) ) {
					$testimonials_category_title_display = esc_html__( 'Category', 'noizzy-core' ) . ': ' . $testimonials_category_title;
				}
