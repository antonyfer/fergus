...
/*
   Class: NoizzyEdgeMultipleImages
   A class that initializes Edge Multiple Images
*/

class NoizzyEdgeMultipleImages implements iNoizzyEdgeRender {
	private $name;
	private $label;
	private $description;
	...
	function __construct( $name, $label = "", $description = "" ) {
		...
		$noizzy_edge_Framework->edgeMetaBoxes->addOption( $this->name, "" );
	}
					...
					<div class="row">
						<div class="col-lg-12">
							...
							<div class="edge-gallery-uploader">
								<a class="edge-gallery-upload-btn btn btn-sm btn-primary" href="javascript:void(0)"><?php esc_html_e( 'Upload', 'noizzy' ); ?></a>
								<a class="edge-gallery-clear-btn btn btn-sm btn-default pull-right" href="javascript:void(0)"><?php esc_html_e( 'Remove All', 'noizzy' ); ?></a>
							</div>
...
class NoizzyEdgeTwitterFramework implements iNoizzyEdgeRender {
	public function render( $factory ) {
		$twitterApi = NoizzyTwitterApi::getInstance();
		$message    = '';
		...
		if ( ! empty( $_GET['oauth_token'] ) && ! empty( $_GET['oauth_verifier'] ) ) {
			...
			if ( $responseObj->status ) {
				$message = esc_html__( 'You have successfully connected with your Twitter account. If you have any issues fetching data from Twitter try reconnecting.', 'noizzy' );
			} else {
				$message = $responseObj->message;
			}
		}
		
		$buttonText = $twitterApi->hasUserConnected() ? esc_html__( 'Re-connect with Twitter', 'noizzy' ) : esc_html__( 'Connect with Twitter', 'noizzy' );
		?>
		<?php if ( $message !== '' ) { ?>
		...
		<div class="edge-page-form-section" id="edge_enable_social_share">
			<div class="edge-field-desc">
				<h4><?php esc_html_e( 'Connect with Twitter', 'noizzy' ); ?></h4>
				<p><?php esc_html_e( 'Connecting with Twitter will enable you to show your latest tweets on your site', 'noizzy' ); ?></p>
			</div>
			<div class="edge-section-content">
				<div class="container-fluid">
					<div class="row">
...
class NoizzyEdgeInstagramFramework implements iNoizzyEdgeRender {
    public function render( $factory ) {
        ...
        if ( ! empty( $_GET['code'] ) && ! empty( $_GET['instagram'] ) ) {
            ...
            $message = esc_html__( 'You have successfully connected with your Instagram Personal account.', 'noizzy' );
        }
        ...
        if ( ! empty( $_GET['access_token'] ) && ! empty( $_GET['facebook'] ) ) {
            ...
            $message = esc_html__( 'You have successfully connected with your Instagram Business account.', 'noizzy' );
        }
        ...
        if ( ! empty( $_GET['disconnect'] ) ) {
            ...
            $message = esc_html__( 'You have have been disconnected from all Instagram accounts.', 'noizzy' );
        ...
        <div class="edge-page-form-section" id="edge_enable_social_share">
            <div class="edge-field-desc">
                <h4><?php esc_html_e( 'Connect with Instagram', 'noizzy' ); ?></h4>
                <p><?php esc_html_e( 'Connecting with Instagram will enable you to show your latest photos on your site', 'noizzy' ); ?></p>
            </div>
            <div class="edge-section-content">
                <div class="container-fluid">
                    <?php
                    ...
                    if ( $instagram_user_id ) { ?>
                        <div class="row">
                            <div class="col-lg-12">
                                <p><?php echo esc_html__( 'You are currently connected to Instagram ID: ', 'noizzy' );
                    ...
                    <div class="row">
                        <?php if ( ! empty( $_GET['disconnect'] ) ) { ?>
                            <div class="col-lg-4">
                                <a class="btn btn-primary" href="<?php echo esc_url( $instagram_api->reloadURL() ); ?>"><?php echo esc_html__( 'Reload Page', 'noizzy' ); ?></a>
                            </div>
                        <?php } else if ( empty( $connection_type ) ) { ?>
                            <div class="col-lg-4">
                                <a class="btn btn-primary" href="<?php echo esc_url( $instagram_api->instagramRequestCode() ); ?>"><?php echo esc_html__( 'Connect with Instagram Personal account', 'noizzy' ); ?></a>
                            </div>
                            <div class="col-lg-4">
                                <a class="btn btn-primary" href="<?php echo esc_url( $instagram_api->facebookRequestCode() ); ?>"><?php echo esc_html__( 'Connect with Instagram Business account', 'noizzy' ); ?></a>
                            </div>
                        <?php } else { ?>
                            <div class="col-lg-4">
                                <a class="btn btn-primary" href="<?php echo esc_url( $instagram_api->disconnectURL() ); ?>"><?php echo esc_html__( 'Disconnect Instagram account', 'noizzy' ) ?></a>
                            </div>
                        <?php } ?>
                    </div>
                </div>
...
class NoizzyEdgeRepeater implements iNoizzyEdgeRender {
	private $label;
	private $description;
	private $name;
	private $fields;
	...
	public function render( $factory ) {
		...
		if ($this->table_layout){
			$wrapper_classes[] = 'edge-repeater-table';
		}

		?>
		<div class="edge-repeater-wrapper <?php echo implode(' ', $wrapper_classes)?>">
			<?php  if ( $this->label !== '') { ?>
			...
			<?php if ($this->table_layout) { ?>
				<div class="edge-repeater-table-heading">
					<div class="edge-repeater-fields-holder">
						<div class="edge-repeater-table-cell edge-repeater-sort"><?php esc_html_e( 'Order', 'noizzy' ) ?></div>
						<div class="edge-repeater-fields">
							<?php foreach ( $this->fields as $field ) { 
								$col_width_class = 'col-xs-12';
								if ( ! empty($field['col_width']) ) {
									$col_width_class = 'col-xs-'.$field['col_width'];
								} ?>
								<div class="edge-repeater-table-cell <?php echo esc_attr($col_width_class);?>"><?php echo esc_html( $field['th'] ); ?></div>
							<?php } ?>
						</div>
						<div class="edge-repeater-table-cell edge-repeater-remove"><?php esc_html_e( 'Remove', 'noizzy' ) ?></div>
					</div>
				</div>
			<?php } ?>
			<div class="edge-repeater-wrapper-inner <?php echo esc_attr( $sortable_class ); ?>" data-template="<?php echo str_replace('_', '-', $this->name); ?>">
				<?php if (! empty($clones) && count($clones) > 0) {
					$counter = 0;
					foreach($clones as $clone) {
					?>
					<div class="edge-repeater-fields-holder clearfix" data-index="<?php echo esc_attr($counter); ?>">
						<div class="edge-repeater-sort">
						...
						<div class="edge-repeater-fields">
							<?php
								foreach ($this->fields as $field) {
									...
									?>
									<div class="edge-repeater-fields-row <?php echo esc_attr($col_width_class);?>">
										<div class="edge-repeater-fields-row-inner">
										<?php
											if($field['type'] == 'repeater'){ 
												...
												<div class="edge-repeater-inner-wrapper">
													<div class="edge-repeater-inner-wrapper-inner <?php echo esc_attr( $sortable_inner_class ); ?>" data-template="<?php echo str_replace('_', '-', $field['name']); ?>">
														...
														<?php if (!empty($clone[$field['name']]) && count($clone[$field['name']]) > 0) {
															...
															foreach($clone[$field['name']] as $clone_inner) {
																?>
																<div class="edge-repeater-inner-fields-holder edge-second-level clearfix" data-index="<?php echo esc_attr($counter2); ?>">
																	<div class="edge-repeater-sort">
																		<i class="fa fa-sort"></i>
																	</div>
																	<div class="edge-repeater-inner-fields">
																		<?php
																		foreach ($field['fields'] as $field_inner) { 
																			...
																			<div class="edge-repeater-inner-fields-row <?php echo esc_attr( $col_width_inner_class ); ?>">
																				<div class="edge-repeater-inner-fields-row-inner">
																					<?php
																					...
																					if ( ! empty( $field_inner['dependency'] ) ) {
																						$dependencyValues = noizzy_edge_return_repeater_dependency_options_array(array(
																							'field' 	   => $field,
																	...
																	<div class="edge-repeater-remove">
													...
													<div class="edge-repeater-inner-add">
														<a class="edge-inner-clone btn btn-sm btn-primary" data-count="<?php echo esc_attr($this->num_of_rows) ?>" href="#"><?php echo esc_html($field['button_text']); ?></a>
													</div>
												</div>
											<?php
											} else {
				...
				<?php $counter++; } } ?>
				<script type="text/html" id="tmpl-edge-repeater-template-<?php echo str_replace('_', '-', $this->name); ?>">
					<div class="edge-repeater-fields-holder <?php echo esc_attr( $sortable_class ); ?> clearfix"  data-index="{{{ data.rowIndex }}}">
						<div class="edge-repeater-sort">
							<i class="fa fa-sort"></i>
						</div>
						<div class="edge-repeater-fields">
							...
							foreach ($this->fields as $field) { 
								...
								<div class="edge-repeater-fields-row <?php echo esc_attr($col_width_class);?>">
									<div class="edge-repeater-fields-row-inner">
										...
										if($field['type'] == 'repeater') { ?>
											<div class="edge-repeater-inner-wrapper">
												<div class="edge-repeater-inner-wrapper-inner" data-template="<?php echo str_replace('_', '-', $field['name']); ?>">
													<h4><?php echo esc_attr($field['label']); ?></h4>
													<?php if($field['description'] != ''){ ?>
														<p><?php echo esc_attr($field['description']); ?></p>
													<?php } ?>
												...
												<div class="edge-repeater-inner-add">
													<a class="edge-inner-clone btn btn-sm btn-primary" data-count="<?php echo esc_attr($this->num_of_rows) ?>" href="#">
				...
				foreach ($this->fields as $field) {
					if($field['type'] == 'repeater') {
					?>
					<script type="text/html" id="tmpl-edge-repeater-inner-template-<?php echo str_replace('_', '-', $field['name']); ?>">
						<div class="edge-repeater-inner-fields-holder edge-second-level clearfix" data-index="{{{ data.rowInnerIndex }}}">
							<div class="edge-repeater-sort">
								<i class="fa fa-sort"></i>
							</div>
							<div class="edge-repeater-inner-fields">
...
class NoizzyEdgeFieldAddress extends NoizzyEdgeFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $repeat = array() ) {
		...
		<div class="edge-page-form-section edge-address-field <?php echo esc_attr( $class ); ?>" data-country="<?php echo esc_attr( $country ); ?>" data-lat-field="<?php echo esc_attr( $lat_field ); ?>" data-long-field="<?php echo esc_attr( $long_field ); ?>" id="edge_<?php echo esc_attr( $id ); ?>">
			<div class="edge-field-desc">
					...
					<div class="row">
						<div class="col-lg-<?php echo esc_attr( $col_width ); ?>">
							...
							<div class="input-group">
								<?php endif; ?>
								<input type="text" class="form-control edge-input edge-form-element" name="<?php echo esc_attr( $name ); ?>" value="<?php echo esc_attr( htmlspecialchars( $value ) ); ?>"/>
								<?php if ( $suffix ) : ?>
						...
						<?php endif; ?>
							...
							<button id="find" class="btn btn-primary"><?php esc_html_e( 'Place the pin on the map', 'noizzy' ); ?></button>
...
class NoizzyEdgeFieldIcon extends NoizzyEdgeFieldType {
    public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $repeat = array() ) {
        $class = '';
        ...
        $icon_packs        = \NoizzyEdgeIconCollections::get_instance()->getIconCollectionsEmpty();
        ...
        <div class="edge-page-form-section <?php echo esc_attr($class); ?>" id="edge_<?php echo esc_attr($id); ?>">
            ...
            <div class="edge-section-content">
                <div class="container-fluid">
                    <div class="row">
                        <div class="col-lg-<?php echo esc_attr($col_width); ?>">
                            <select name="<?php echo esc_attr( $name ) . '[icon_pack]'; ?>" class="<?php echo esc_attr($select2) ?> form-control edge-form-element icon-dependence">
                    ...
                    <?php foreach ( $icons_collections as $icons_collection ) { ?>
                        ...
                        ?>
                        <div class="row edge-icon-collection-holder" style="<?php echo esc_attr( $style ); ?>" data-icon-collection="<?php echo esc_attr( $icons_collection ); ?>">
                            <div class="col-lg-<?php echo esc_attr($col_width); ?>">
                                <select name="<?php echo esc_attr( $name . '[' . $icons_param . ']'); ?>" class="<?php echo esc_attr($select2) ?> form-control edge-form-element">
                                    <?php
