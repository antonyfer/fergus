...
/*
   Class: NoizzyEdgeFramework
   A class that initializes Edge Framework
*/
class NoizzyEdgeFramework {
	private static $instance;
	public $edgeOptions;
	public $edgeMetaBoxes;
	public $edgeTaxonomyOptions;
	public $edgeUserOptions;
	private $skin;
	
	private function __construct() {
		$this->edgeOptions         = NoizzyEdgeOptions::get_instance();
		$this->edgeMetaBoxes       = NoizzyEdgeMetaBoxes::get_instance();
		$this->edgeTaxonomyOptions = NoizzyEdgeTaxonomyOptions::get_instance();
		$this->edgeUserOptions     = NoizzyEdgeUserOptions::get_instance();
		$this->edgeDashboardOptions = NoizzyEdgeDashboardOptions::get_instance();
	}
	...
	public function setSkin( NoizzyEdgeSkinAbstract $skinObject ) {
		$this->skin = $skinObject;
	}
}

/**
 * Class NoizzyEdgeSkinManager
 *
...
class NoizzyEdgeSkinManager {
	...
	/**
	 * @see NoizzyEdgeSkinManager::setSkin()
	 */
	...
	private function setSkin( $skinName = EDGE_PROFILE_SLUG ) {
		if ( $skinName !== '' ) {
			if ( file_exists( get_template_directory() . '/framework/admin/skins/' . $skinName . '/skin.php' ) ) {
				...
				$skinName = ucfirst( $skinName ) . esc_html__( 'Skin', 'noizzy' );
...
/**
 * Class NoizzyEdgeSkinAbstract
 *
 * Abstract class that each skin class must extend
 */
abstract class NoizzyEdgeSkinAbstract {
	/**
	 * @var string
	 */
	protected $skinName;
	...
	public function loadTemplatePart( $template, $params = array() ) {
		...
		if ( $template !== '' ) {
			include( noizzy_edge_find_template_path( 'framework/admin/skins/' . $this->skinName . '/templates/' . $template . '.php' ) );
		}
	}
	...
	public function setShortcodeJSParams() { ?>
		<script>
			window.edgeSCIcon = '<?php echo noizzy_edge_get_skin_uri() . '/assets/img/admin-logo-icon.png'; ?>';
			window.edgeSCLabel = '<?php echo esc_html( ucfirst( $this->skinName ) ); ?> <?php esc_html_e( 'Shortcodes', 'noizzy' ) ?>';
		</script>
	<?php }
...
class NoizzyEdgeOptions {
	...
	public function getOptionValue( $key ) {
		...
		if ( array_key_exists( $key, $noizzy_edge_options ) ) {
			return $noizzy_edge_options[ $key ];
...
/*
   Class: NoizzyEdgeAdminPage
   A class that initializes Edge Admin Page
*/
class NoizzyEdgeAdminPage implements iNoizzyEdgeLayoutNode {
	public $layout;
	private $factory;
	public $slug;
	public $title;
	...
	function __construct( $slug = "", $title = "", $icon = "" ) {
		$this->layout  = array();
		$this->factory = new NoizzyEdgeFieldFactory();
		$this->slug    = $slug;
		$this->title   = $title;
		$this->icon    = $icon;
	}
	...
	public function renderChild( iNoizzyEdgeRender $child ) {
		$child->render( $this->factory );
	}
}

/*
   Class: NoizzyEdgeMetaBoxes
...
/*
   Class: NoizzyEdgeMetaBox
   A class that initializes Edge Meta Box
*/
class NoizzyEdgeMetaBox implements iNoizzyEdgeLayoutNode {
	public $layout;
	private $factory;
	public $scope;
	public $title;
	...
	public function setTitle( $label ) {
		...
		return $noizzy_edge_Framework->getSkin()->getSkinLabel() . ' ' . $label;
	}
}

/*
   Class: NoizzyEdgeTaxonomyOptions
   A class that initializes NoizzyEdge Taxonomy Options
*/
class NoizzyEdgeTaxonomyOptions {
	private static $instance;
...
/*
   Class: NoizzyEdgeTaxonomyOption
   A class that initializes NoizzyEdge Taxonomy Option
*/
class NoizzyEdgeTaxonomyOption implements iNoizzyEdgeLayoutNode {
	public $layout;
	private $factory;
	public $scope;
	...
	function __construct( $scope = "" ) {
		$this->layout  = array();
		$this->factory = new NoizzyEdgeTaxonomyFieldFactory();
		$this->scope   = $scope;
	}
...
/*
   Class: NoizzyEdgeUserOptions
   A class that initializes NoizzyEdge User Options
*/
class NoizzyEdgeUserOptions {
	private static $instance;
	public $userOptions;
...
/*
   Class: NoizzyEdgeUserOption
   A class that initializes NoizzyEdge User Option
*/
class NoizzyEdgeUserOption implements iNoizzyEdgeLayoutNode {
	public $layout;
	private $factory;
	public $scope;
	...
	function __construct( $scope = "" ) {
		$this->layout  = array();
		$this->factory = new NoizzyEdgeUserFieldFactory();
		$this->scope   = $scope;
	}
...
/*
   Class: NoizzyEdgeDashboardOptions
   A class that initializes NoizzyEdge Dashboard Options
*/
class NoizzyEdgeDashboardOptions {
	private static $instance;
...
/*
   Class: NoizzyEdgeDashboardOption
   A class that initializes NoizzyEdge Dashboard Option
*/
class NoizzyEdgeDashboardOption implements iNoizzyEdgeLayoutNode {
	public $layout;
	private $factory;
	
	function __construct() {
		$this->layout        = array();
		$this->factory       = new NoizzyEdgeDashboardFieldFactory();
	}
...
if ( ! function_exists( 'noizzy_edge_init_framework_variable' ) ) {
	function noizzy_edge_init_framework_variable() {
		global $noizzy_edge_Framework;
		
		$noizzy_edge_Framework = NoizzyEdgeFramework::get_instance();
		$edgeSkinManager                 = new NoizzyEdgeSkinManager();
		$noizzy_edge_Framework->setSkin( $edgeSkinManager->getSkin() );
	}
	
	add_action( 'noizzy_edge_before_options_map', 'noizzy_edge_init_framework_variable' );
}
?>