...
include EDGE_FRAMEWORK_ROOT_DIR . "/lib/edge.icons/edge.iconcollection.interface.php";
include EDGE_FRAMEWORK_ROOT_DIR . "/lib/edge.icons/edge.fontawesome.php";
include EDGE_FRAMEWORK_ROOT_DIR . "/lib/edge.icons/edge.fontelegant.php";
include EDGE_FRAMEWORK_ROOT_DIR . "/lib/edge.icons/edge.ionicons.php";
include EDGE_FRAMEWORK_ROOT_DIR . "/lib/edge.icons/edge.lineaicons.php";
include EDGE_FRAMEWORK_ROOT_DIR . "/lib/edge.icons/edge.linearicons.php";
include EDGE_FRAMEWORK_ROOT_DIR . "/lib/edge.icons/edge.simplelineicons.php";
include EDGE_FRAMEWORK_ROOT_DIR . "/lib/edge.icons/edge.dripicons.php";

/*
  Class: NoizzyEdgeIconCollections
...
class NoizzyEdgeIconCollections {
    ...
    private function initIconCollections() {
        $this->addIconCollection('font_awesome', new NoizzyEdgeIconsFontAwesome("Font Awesome", "fa_icon"));
        $this->addIconCollection('font_elegant', new NoizzyEdgeIconsFontElegant("Font Elegant", "fe_icon"));
        $this->addIconCollection('ion_icons', new NoizzyEdgeIonIcons("Ion Icons", "ion_icon"));
        $this->addIconCollection('linea_icons', new NoizzyEdgeLineaIcons('Linea Icons', 'linea_icon'));
        $this->addIconCollection('linear_icons', new NoizzyEdgeLinearIcons('Linear Icons', 'linear_icon'));
        $this->addIconCollection('simple_line_icons', new NoizzyEdgeSimpleLineIcons('Simple Line Icons', 'simple_line_icon'));
        $this->addIconCollection('dripicons', new NoizzyEdgeDripicons('Dripicons', 'dripicon'));
    }
    ...
    public function getIconsMetaBoxOrOption($attributes) {
	    ...
	    $options = array(
		    'font_awesome'      => esc_html__( 'Font Awesome', 'noizzy' ),
		    'font_elegant'      => esc_html__( 'Font Elegant', 'noizzy' ),
		    'ion_icons'         => esc_html__( 'Ion Icons', 'noizzy' ),
		    'simple_line_icons' => esc_html__( 'Simple Line Icons', 'noizzy' )
	    );
	    ...
	    if ( $scope == 'regular' ) {
		    $options = array(
			    ...
			    'font_elegant'      => esc_html__( 'Font Elegant', 'noizzy' ),
			    'ion_icons'         => esc_html__( 'Ion Icons', 'noizzy' ),
			    'linea_icons'       => esc_html__( 'Linea Icons', 'noizzy' ),
			    'linear_icons'      => esc_html__( 'Linear Icons', 'noizzy' ),
			    'simple_line_icons' => esc_html__( 'Simple Line Icons', 'noizzy' ),
			    'dripicons'         => esc_html__( 'Dripicons', 'noizzy' )
		    );
	    }
	    ...
	    foreach ( $icon_collections as $collection_key => $collection_object ) {
		    ...
		    $edge_icon_hide_values = $icon_collections_keys;
		    array_push( $edge_icon_hide_values, '' ); //add empty value for icon switcher
		
		    $edge_icon_pack_container = noizzy_edge_add_admin_container(
			    array(
				    'parent'     => $parent,
		    ...
		    if ( $type == 'meta-box' ) {
			    noizzy_edge_create_meta_box_field(
				    array(
					    'parent'        => $edge_icon_pack_container,
					    'type'          => 'select' . $field_type,
	...
	public function getVCParamsArray( $iconPackDependency = array(), $iconCollectionPrefix = "", $emptyIconPack = false ) {
		...
		$iconPackParams = array(
			'type'        => 'dropdown',
			'heading'     => esc_html__( 'Icon Pack', 'noizzy' ),
		...
		if ( is_array( $this->iconCollections ) && count( $this->iconCollections ) ) {
			foreach ( $this->iconCollections as $key => $collection ) {
				$iconSetParams[] = array(
					'type'        => 'dropdown',
					'heading'     => esc_html__( 'Icon', 'noizzy' ),
	...
	public function getIconWidgetParamsArray() {
		...
		$iconPackParams[] = array(
			'type'    => 'dropdown',
			'name'    => 'icon_pack',
			'title'   => esc_html__( 'Icon Pack', 'noizzy' ),
			'options' => array(
				'font_awesome'      => esc_html__( 'Font Awesome', 'noizzy' ),
				'font_elegant'      => esc_html__( 'Font Elegant', 'noizzy' ),
				'ion_icons'         => esc_html__( 'Ion Icons', 'noizzy' ),
		...
		if ( is_array( $this->iconCollections ) && count( $this->iconCollections ) ) {
			foreach ( $this->iconCollections as $key => $collection ) {
				$iconSetParams[] = array(
					'type'    => 'dropdown',
					'title'   => $collection->title . esc_html__( ' Icon', 'noizzy' ),
	...
	public function getSocialIconWidgetMultipleParamsArray( $count ) {
		...
		for ( $n = 1; $n <= $count; $n ++ ) {
			...
			if ( is_array( $iconCollectionsVC ) && count( $iconCollectionsVC ) ) {
				foreach ( $iconCollectionsVC as $key => $collection ) {
					$iconOps[] = array(
						...
						'title'   => sprintf( esc_html__( 'Icon %s %s Icon', 'noizzy' ), $n, $collection->title ),
			...
			$iconOps[] = array(
				'type'  => 'textfield',
				'name'  => 'link_' . $n,
				'title' => sprintf( esc_html__( 'Link %s', 'noizzy' ), $n )
			);
			
			$iconOps[] = array(
				'type'    => 'dropdown',
				'name'    => 'target_' . $n,
				'title'   => sprintf( esc_html__( 'Link Target %s', 'noizzy' ), $n ),
	...
	public function getIconSizeClass( $iconSize ) {
		switch ( $iconSize ) {
			case "fa-lg":
				$iconSize = "edge-tiny-icon";
				break;
			case "fa-2x":
				$iconSize = "edge-small-icon";
				break;
			case "fa-3x":
				$iconSize = "edge-medium-icon";
				break;
			case "fa-4x":
				$iconSize = "edge-large-icon";
				break;
			case "fa-5x":
				$iconSize = "edge-huge-icon";
				break;
			default:
				$iconSize = "edge-small-icon";
		}
	...
	public function getIconCollectionIcons( iNoizzyEdgeIconCollection $collection ) {
	...
	public function getIconCollectionsEmpty( $no_empty_key = "" ) {
		$array                  = array();
		$array[ $no_empty_key ] = esc_html__( 'No Icon', 'noizzy' );
	...
	public function getIconCollectionsVCEmpty() {
		$vc_array                                           = array();
		$vc_array[ esc_html__( 'No Icon', 'noizzy' ) ] = '';
	...
	public function enqueueStyles() {
		if ( is_array( $this->iconCollections ) && count( $this->iconCollections ) ) {
			foreach ( $this->iconCollections as $collection_key => $collection_obj ) {
				wp_enqueue_style( 'edge_' . $collection_key, $collection_obj->styleUrl );
			}
		}
	}
...
if ( ! function_exists( 'noizzy_edge_activate_theme_icons' ) ) {
	function noizzy_edge_activate_theme_icons() {
		global $noizzy_edge_IconCollections;
		$noizzy_edge_IconCollections = NoizzyEdgeIconCollections::get_instance();
	}
	
	add_action( 'after_setup_theme', 'noizzy_edge_activate_theme_icons' );
}