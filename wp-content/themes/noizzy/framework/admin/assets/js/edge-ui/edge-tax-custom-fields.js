jQuery(document).ready(function ($) {
    ...
    function edge_tax_media_upload(button_class) {
        ...
        $('body').on('click', button_class, function (e) {
            ...
            wp.media.editor.send.attachment = function (props, attachment) {
                if (_custom_media) {
                    ...
                    parent.find('.edge-tax-custom-media-url').val(attachment.id);
                    parent.find('.edge-tax-image-wrapper').html('<img class="custom_media_image" src="" style="margin:0;padding:0;max-height:100px;float:none;" />');
                    parent.find('.edge-tax-image-wrapper .custom_media_image').attr('src', attachment_url).css('display', 'block');
                } else {
    ...
    function edge_tax_media_remove(button_class) {
        $('body').on('click', button_class, function () {
            var $this = $(this),
                parent = $this.closest('.form-field'),
                image = parent.find('.edge-tax-custom-media-url');
            ...
            if (image.val() !== '' && confirm('Are you sure you want to delete this file?')) {
                var result = $.ajax({
                    ...
                    type: 'GET',
                    data: {
                        action: 'noizzy_select_tax_del_image',
    ...
    function edgeInitTaxColorpicker() {
        var taxColor = $('.edge-taxonomy-color-field');
    ...
    edge_tax_media_upload('.edge-tax-media-add.button');
    edge_tax_media_remove('.edge-tax-media-remove.button');
    edgeInitTaxColorpicker();
        ...
        if (settings.data) {
            ...
            if ($.inArray('action=add-tag', queryStringArr) !== -1) {
                ...
                if ($response !== "") {
                    ...
                    $('.edge-tax-image-wrapper').html('');
                    $('.edge-taxonomy-color-field').val('');
                    $('.edge-taxonomy-color-field').parents('.wp-picker-container').find('.wp-color-result').removeAttr('style');
                }
            }
        }
    });

    edgeInitTermIconSelectChange();

    function edgeInitTermIconSelectChange() {
