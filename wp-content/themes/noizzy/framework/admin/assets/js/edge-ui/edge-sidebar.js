(function($){
	...
	var EdgefSidebar = function(){
		...
		this.widget_add  = $('#edge-add-widget');
    ...
    EdgefSidebar.prototype = {
		...
		create_form: function(){
			...
			this.widget_name = this.widget_wrap.find('input[name="edge-sidebar-widgets"]');
			this.nonce = this.widget_wrap.find('input[name="edge-delete-sidebar"]').val();
		},

		add_del_button: function(){
			this.widget_area.find('.sidebar-edge-custom').append('<span class="edge-delete-button"></span>');
		},

		bind_events: function(){
			this.widget_wrap.on('click', '.edge-delete-button', $.proxy( this.delete_sidebar, this));
		},
		...
		delete_sidebar: function(e){
			...
			$.ajax({
				...
				url: window.ajaxurl,
				data: {
					action: 'edge_ajax_delete_custom_sidebar',
					name: widget_name,
					_wpnonce: obj.nonce
				},
