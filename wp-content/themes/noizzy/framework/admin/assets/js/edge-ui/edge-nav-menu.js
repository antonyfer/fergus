(function($) {
	...
	$(document).ready(function() {
        edgeUpdateIconOptions();
		edgeInitAdditionalItemOptions();
	});

	/**
	 * Function that serializes additional menu item options in a single field.
	 */
	function edgeInitAdditionalItemOptions() {
		var navForm = $('#update-nav-menu');

		navForm.on('change', '[data-item-option]', function() {
			edgeGenerateSerializedString();
		});
	}

	function edgeGenerateSerializedString() {
		var dataArrayString = '',
		...
		var menuOptions = $('input[name="edge_menu_options"]');
		if(menuOptions.length) {
			menuOptions.val(encodeURIComponent(dataArrayString));
		} else {
			//generate hidden input field html with serialized string value
			var hiddenMenuItem = '<input type="hidden" name="edge_menu_options" value="'+encodeURIComponent(dataArrayString)+'">';
     ...
     */
    function edgeUpdateIconOptions() {
        var navForm = $('#update-nav-menu');

        navForm.on('change', '[data-icon-pack]', function() {
	        var thisItem = $(this),
		        chosenIconPack = thisItem.find('option:selected').val(),
		        iconDropdown = thisItem.parents('p').first().next('.edge-icon-select-holder').find('select'),
