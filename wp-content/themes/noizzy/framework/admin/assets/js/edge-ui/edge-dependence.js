(function ($) {
	...
	window.edgeDependencyAdmin = {};
	
	edgeDependencyAdmin.edgeReInitOptionsDependency = edgeReInitOptionsDependency;
	
	$(document).ready(function () {
		edgeInitOptionsDependency().init();
		edgeInitSelectChange();
		edgeInitIconSelectChange();
		edgeInitRadioChange();
	});
	
	function edgeReInitOptionsDependency() {
		edgeInitOptionsDependency().init(true);
	}
	
	var edgeInitOptionsDependency = function () {
		...
		function multipleDependencyLogic(items, optionHolder, optionName, optionValue, dependencyType) {
			...
			$.each(items, function (key, value) {
				...
				if (optionName === key) {
					...
					var otherOptionType = $('.edge-dependency-option[data-option-name="' + key + '"]').data('option-type');
					switch (otherOptionType) {
						case 'checkbox':
							var otherValue = $('.edge-dependency-option[data-option-name="' + key + '"]').find('input[type="hidden"][name="' + key + '"]').val();
							break;
						case 'selectbox':
							var otherValue = $('.edge-dependency-option[data-option-name="' + key + '"]').val();
							break;
					}
		...
		function mainLogic(thisItem, optionValue) {
			var dependencyHolder = $('.edge-dependency-holder'),
		...
		function checkBox(thisItem, repeater) {
			...
			if (repeater) {
				...
				if (thisItem.parents('.edge-repeater-fields-holder').length && repeaterOptionValue !== undefined) {
		...
		function selectBox(thisItem, repeater) {
			if (repeater && thisItem.parents('.edge-repeater-fields-holder').length) {
		...
		return {
			init: function (repeater) {
				var dependencyOption = $('.edge-section-content .edge-field[data-option-name]');
				...
				if (dependencyOption.length) {
					dependencyOption.each(function () {
						...
						thisOptions.addClass('edge-dependency-option');
    ...
    function edgeInitIconSelectChange() {
        $(document).on('change', 'select.icon-dependence', function (e) {
            var valueSelected = this.value.replace(/ /g, ''),
            	parentSection = $(this).parents('.edge-section-content');

            parentSection.find('.row.edge-icon-collection-holder').fadeOut();
            parentSection.find('.row.edge-icon-collection-holder[data-icon-collection="' + valueSelected + '"]').fadeIn();
        });
    }
	
	function edgeInitRadioChange() {
