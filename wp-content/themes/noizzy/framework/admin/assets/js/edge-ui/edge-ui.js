(function($){
	...
	edgeUIAdmin.edgeInitDatePicker = edgeInitDatePicker;
	edgeUIAdmin.edgeSelect2 = edgeSelect2;
	edgeUIAdmin.edgeInitSwitch = edgeInitSwitch;
	edgeUIAdmin.edgeInitMediaUploader = edgeInitMediaUploader;
	edgeUIAdmin.edgeInitColorpicker = edgeInitColorpicker;
	...
	$(document).ready(function () {
        ...
        edgeInitSelectSwitcherChange();
        edgeInitSwitch();
        edgeInitSaveCheckBoxesValue();
        edgeCheckBoxMultiSelectInitState();
        edgeInitCheckBoxMultiSelectChange();
        edgeCheckVisibilityOfAnchorSelectOptions();
	...
	function edgeInitColorpicker() {
		var holder = $('.edge-page .my-color-field');
	...
	function edgeInitMediaUploaderAdded(addButton) {
		addButton.siblings('.edge-portfolio-media:last').find('.edge-media-uploader').each(function(){
			var thisItem = $(this),
				fileFrame,
				uploadUrl,
				uploadHeight,
			...
			uploadUrl           = thisItem.find('.edge-media-upload-url');
			uploadHeight        = thisItem.find('.edge-media-upload-height');
			uploadWidth        = thisItem.find('.edge-media-upload-width');
			uploadImageHolder   = thisItem.find('.edge-media-image-holder');
			removeButton        = thisItem.find('.edge-media-remove-btn');
			...
			thisItem.on('click', '.edge-media-upload-btn', function() {
	...
	function edgeInitDatePicker() {
		var holder = $('.edge-input.datepicker');
			...
			holder.each(function () {
				...
				thisItem.noUiSlider({
					...
					serialization: {
						lower: [
							new Link({
								target: thisItem.prev('.edge-slider-range-value')
							})
						],
						format: {
							// Set formatting
	...
	function edgeInitMediaUploader() {
		var holder = $('.edge-media-uploader');
	...
	function edgeInitGalleryUploader() {
		var $edge_upload_button = jQuery('.edge-gallery-upload-btn'),
			$edge_clear_button = jQuery('.edge-gallery-clear-btn');
		...
		wp.media.customlibEditGallery1 = {
			frame: function () {
				...
				this._frame.on('update', function () {
					...
					var data = {
						action: 'noizzy_edge_gallery_upload_get_images',
						ids: ids,
	...
	function edgeInitSortable() {
		var sortingHolder = $('.edge-sortable-holder'),
			enableParentChild = sortingHolder.hasClass('edge-enable-pc');
	...
	function edgeSelect2() {
		var holder = $('select.edge-select2');
	...
	function edgeInitGeocomplete() {
		var geo_inputs = $(".edge-address-field");
		
		if (geo_inputs.length && !$('body').hasClass('edge-empty-google-api')) {
	...
	function edgeInitAjaxForm() {
		$('#edge_top_save_button').on('click',function () {
			...
			var inputChangeClass = $('.edge-input-change.yes'),
				changesClass = $('.edge-changes-saved');
		...
		$(document).delegate(".edge_ajax_form", "submit", function (a) {
			var b = $(this),
				c = {
					action: "noizzy_edge_save_options"
				};
	...
	function edgeImportOptions() {
		var holder = $('.edge-backup-options-page-holder');
			...
			edgeImportBtn.on('click', function (e) {
				...
				if (confirm('Are you sure, you want to import Options now?')) {
					...
					var importValue = $('#import_theme_options').val(),
						importNonce = $('#edge_import_theme_options_secret').val();
					
					var data = {
						action: 'noizzy_core_import_theme_options',
						content: importValue,
						nonce: importNonce
					};
			...
			edgeImportBtn.on('click', function (e) {
				...
				if (confirm('Are you sure, you want to import Custom Sidebars now?')) {
					...
					var importValue = $('#import_custom_sidebars').val(),
						importNonce = $('#edge_import_custom_sidebars_secret').val();
					
					var data = {
						action: 'noizzy_core_import_custom_sidebars',
						content: importValue,
						nonce: importNonce
					};
			...
			edgeImportBtn.on('click', function (e) {
				...
				if (confirm('Are you sure, you want to import Widgets now?')) {
					...
					var importValue = $('#import_widgets').val(),
						importNonce = $('#edge_import_widgets_secret').val();
					
					var data = {
						action: 'noizzy_core_import_widgets',
						content: importValue,
						nonce: importNonce
					};
	...
	function edgeInitImportContent() {
		...
		if (edgeImportHolder.length) {
			var edgeImportBtn = $('#edge-import-demo-data'),
				confirmMessage = edgeImportHolder.data('confirm-message');
			...
			edgeImportBtn.on('click', function (e) {
				...
				if (confirm(confirmMessage)) {
					...
					if (import_opt === 'content') {
						for (var i = 1; i < 10; i++) {
							var str = i < 10 ? 'noizzy_content_0' + i + '.xml' : 'noizzy_content_' + i + '.xml';
							
							jQuery.ajax({
								type: 'POST',
								url: ajaxurl,
								data: {
									action: 'noizzy_core_data_import',
									xml: str,
									example: import_expl,
						...
						$.ajax({
							...
							url: ajaxurl,
							data: {
								action: 'noizzy_core_widgets_import',
								example: import_expl
							},
							success: function (data, textStatus, XMLHttpRequest) {
						...
						jQuery.ajax({
							...
							url: ajaxurl,
							data: {
								action: 'noizzy_core_options_import',
								example: import_expl
							},
							success: function (data, textStatus, XMLHttpRequest) {
								$('.progress-value').html((100) + '%');
						...
						for (var i = 1; i < 10; i++) {
							var str = i < 10 ? str = 'noizzy_content_0' + i + '.xml' : 'noizzy_content_' + i + '.xml';
										...
										jQuery.ajax({
											...
											success: function (data, textStatus, XMLHttpRequest) {
												jQuery.ajax({
													...
													url: ajaxurl,
													data: {
														action: 'noizzy_core_other_import',
														example: import_expl
													},
													success: function (data, textStatus, XMLHttpRequest) {
														//alert(data);
    ...
    function edgeInitSelectSwitcherChange() { //used in lms plugin
    ...
    function edgeInitSaveCheckBoxesValue(){
        var checkboxes = $('.edge-single-checkbox-field');
        checkboxes.change(function(){
            edgeDisableHidden($(this));
        });
        checkboxes.each(function(){
            edgeDisableHidden($(this));
        });
        function edgeDisableHidden(thisBox){
            if(thisBox.is(':checked')){
                thisBox.siblings('.edge-checkbox-single-hidden').prop('disabled', true);
            }else{
                thisBox.siblings('.edge-checkbox-single-hidden').prop('disabled', false);
            }
        }
    }

    function edgeCheckBoxMultiSelectInitState() {
        var element = $('input[type="checkbox"].dependence.multiselect');

        if (element.length) {
            element.each(function () {
                var thisItem = $(this);
                edgeInitCheckBox(thisItem);
            });
        }
    }

    function edgeInitCheckBoxMultiSelectChange() {
        var element = $('input[type="checkbox"].dependence.multiselect');
    ...
    function edgeInitCheckBox(checkBox) {
    ...
    function edgeCheckVisibilityOfAnchorSelectOptions() {
        var holder = $('.edge-page-form > div:hidden');
    ...
    function edgeShowHideContainersAndAnchorsSelectOptions(){
        setTimeout(function(){
            $('#edge-select-anchor option').show();
            $('.edge-page-form-section-holder.edge-dependency-holder').each(function(){
                var $this = $(this);
                var $id = $this.attr('id');
                if(!$this.is(':visible')){
                    $('#edge-select-anchor option').each ( function() {
    ...
    function edgeCheckOptionAnchorsOnDependencyChange() {
        $(document).on('click','.edge-dependency-option .cb-enable, .edge-dependency-option .cb-disable',function(){
            edgeShowHideContainersAndAnchorsSelectOptions();
        });

        $(document).on('change','.edge-dependency-option input[type=radio]',function(){
            edgeShowHideContainersAndAnchorsSelectOptions()
        });

        $(document).on('change','.edge-form-element.edge-dependency-option',function(){
            edgeShowHideContainersAndAnchorsSelectOptions();
        });
    }
