...
namespace NoizzyEdge\Modules\Header\Lib;
/**
 * Class NoizzyEdgeHeaderConnector
 *
...
class NoizzyEdgeHeaderConnector {
	...
	public function connect( $config = array() ) {
		do_action( 'noizzy_edge_pre_header_connect' );
		...
		if ( ! empty( $config['affect_content'] ) ) {
			add_filter( 'noizzy_edge_content_elem_style_attr', array( $this, 'contentMarginFilter' ) );
		}
		
		if ( ! empty( $config['affect_title'] ) ) {
			add_filter( 'noizzy_edge_title_content_padding', array( $this, 'titlePaddingFilter' ) );
		}
		
		do_action( 'noizzy_edge_after_header_connect' );
	}
	 ...
	 *
	 * Hooks to noizzy_edge_title_content_padding filter
	 *
	 * @return int
	 */
