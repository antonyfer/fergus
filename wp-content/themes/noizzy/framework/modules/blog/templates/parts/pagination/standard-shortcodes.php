<?php if($max_num_pages > 1) { ?>
	...
	<?php
		...
		if($number_of_pages > 1){ ?>
			<div class="edge-bl-standard-pagination">
				<ul>
					<li class="edge-bl-pag-prev">
					...
					<?php for ($i=1; $i <= $number_of_pages; $i++) { ?>
						<?php
							$active_class = '';
							if($current_page == $i) {
								$active_class = 'edge-bl-pag-active';
							}
						?>
						<li class="edge-bl-pag-number <?php echo esc_attr($active_class); ?>">
							<a href="#" data-paged="<?php echo esc_attr($i); ?>"><?php echo esc_html($i); ?></a>
						</li>
					<?php } ?>
					<li class="edge-bl-pag-next">
