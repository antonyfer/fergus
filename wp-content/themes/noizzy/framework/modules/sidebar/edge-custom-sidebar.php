 ...
 */
if ( ! class_exists( 'NoizzyEdgeSidebar' ) ) {
	class NoizzyEdgeSidebar {
		var $sidebars = array();
		var $stored = "";
		...
		function __construct() {
			...
			$this->title  = esc_html__( 'Custom Widget Area', 'noizzy' );
		...
		function load_assets() {
			...
			wp_enqueue_script( 'edge-sidebar', EDGE_ROOT . '/framework/admin/assets/js/edge-ui/edge-sidebar.js' );
		...
		function template_add_widget_field() {
			$nonce = '<input type="hidden" name="edge-delete-sidebar" value="' . wp_create_nonce('edge-delete-sidebar') . '" />';
			
			echo "\n<script type='text/html' id='edge-add-widget'>";
			echo "\n  <form class='edge-add-widget' method='POST'>";
			echo "\n  <h3>" . esc_html($this->title) . "</h3>";
			echo "\n    <span class='input_wrap'><input type='text' value='' placeholder = '" . esc_attr__('Enter Name of the new Widget Area', 'noizzy') . "' name='edge-add-widget' /></span>";
			echo "\n    <input class='button' type='submit' value='" . esc_attr__('Add Widget Area', 'noizzy') . "' />";
			echo "\n    " . $nonce;
			echo "\n  </form>";
			echo "\n</script>\n";
		}
		...
		function register_custom_sidebars() {
			...
			$args = apply_filters( 'noizzy_edge_custom_widget_args', $args );
