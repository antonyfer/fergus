...
class NoizzyEdgeIconWidget extends NoizzyEdgeWidget {
	public function __construct() {
		parent::__construct(
			'edge_icon_widget',
			esc_html__( 'Noizzy Icon Widget', 'noizzy' ),
			array( 'description' => esc_html__( 'Add icons to widget areas', 'noizzy' ) )
		);
	...
	protected function setParams() {
		$this->params = array_merge(
			noizzy_edge_icon_collections()->getIconWidgetParamsArray(),
			array(
				array(
					'type'  => 'textfield',
					'name'  => 'icon_text',
					'title' => esc_html__( 'Icon Text', 'noizzy' )
				),
				array(
					'type'  => 'textfield',
					'name'  => 'link',
				...
				array(
					'type'    => 'dropdown',
					'name'    => 'target',
					'title'   => esc_html__( 'Target', 'noizzy' ),
	...
	public function widget( $args, $instance ) {
		$holder_classes = array( 'edge-icon-widget-holder' );
		if ( ! empty( $instance['icon_hover_color'] ) ) {
			$holder_classes[] = 'edge-icon-has-hover';
		}
		
		$data   = array();
		$data[] = ! empty( $instance['icon_hover_color'] ) ? noizzy_edge_get_inline_attr( $instance['icon_hover_color'], 'data-hover-color' ) : '';
		$data   = implode( ' ', $data );
		...
		if ( ! empty( $instance['icon_size'] ) ) {
			$holder_styles[] = 'font-size: ' . noizzy_edge_filter_px( $instance['icon_size'] ) . 'px';
		}
		...
		if ( ! empty( $instance['icon_pack'] ) ) {
			...
			if ( ! empty( $icon_class ) ) {
				...
				$icon_holder_html = '<span class="edge-icon-element ' . esc_attr( $icon_class ) . '"></span>';
			}
		}
		
		$icon_text_html  = '';
		$icon_text_class = ! empty( $icon_holder_html ) ? '' : 'edge-no-icon';
		if ( ! empty( $instance['icon_text'] ) ) {
			$icon_text_html = '<span class="edge-icon-text ' . esc_attr( $icon_text_class ) . '">' . esc_html( $instance['icon_text'] ) . '</span>';
		}
		?>
		
		<a <?php noizzy_edge_class_attribute( $holder_classes ); ?> <?php echo wp_kses_post( $data ); ?> href="<?php echo esc_url( $link ); ?>" target="<?php echo esc_attr( $target ); ?>" <?php echo noizzy_edge_get_inline_style( $holder_styles ); ?>>
