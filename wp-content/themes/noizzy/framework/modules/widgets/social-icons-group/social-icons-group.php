...
class NoizzyEdgeClassIconsGroupWidget extends NoizzyEdgeWidget {
	public function __construct() {
		parent::__construct(
			'edge_social_icons_group_widget',
			esc_html__( 'Noizzy Social Icons Group Widget', 'noizzy' ),
			array( 'description' => esc_html__( 'Use this widget to add a group of up to 6 social icons to a widget area.', 'noizzy' ) )
		);
	...
	protected function setParams() {
		$this->params = array_merge(
			...
			noizzy_edge_icon_collections()->getSocialIconWidgetMultipleParamsArray(6),
			array(
				array(
					'type'    => 'dropdown',
					'name'    => 'layout',
					'title'   => esc_html__( 'Icons Layout', 'noizzy' ),
					'options' => array(
						''             => esc_html__( 'Default', 'noizzy' ),
						'square-icons' => esc_html__( 'Square', 'noizzy' ),
					)
				),
				array(
					'type'        => 'dropdown',
					'name'        => 'skin',
					'title'       => esc_html__( 'Square Icons Skin', 'noizzy' ),
					'description' => esc_html__( 'This applies to the square layout', 'noizzy' ),
					'options'     => array(
						''           => esc_html__( 'Dark Skin', 'noizzy' ),
						'light-skin' => esc_html__( 'Light Skin', 'noizzy' ),
					)
				),
				array(
					'type'    => 'dropdown',
					'name'    => 'alignment',
					'title'   => esc_html__( 'Text Alignment', 'noizzy' ),
					'options' => array(
                ...
                array(
                    'type'    => 'dropdown',
                    'name'    => 'bottom_border',
                    'title'   => esc_html__( 'Bottom Border', 'noizzy' ),
                    'options' => array(
                        'no'   => esc_html__( 'No', 'noizzy' ),
                        'yes' => esc_html__( 'Yes', 'noizzy' ),
                    )
                ),
				array(
					'type'  => 'textfield',
					'name'  => 'icon_size',
					'title' => esc_html__( 'Icons Size (px)', 'noizzy' )
				),
				array(
					'type'  => 'colorpicker',
					'name'  => 'color',
	...
	public function widget( $args, $instance ) {
		...
		if ( ! empty( $instance['skin'] ) ) {
			$class[] = 'edge-' . $instance['skin'];
		}

		if ( ! empty( $instance['layout'] ) ) {
			$class[] = 'edge-' . $instance['layout'] ;
		}
        ...
        if ( $instance['bottom_border'] === 'yes' ) {
            $class[] = 'edge-has-border';
        }
		...
		if ( ! empty( $instance['icon_size'] ) ) {
			$icon_styles[] = 'font-size: ' . noizzy_edge_filter_px( $instance['icon_size'] ) . 'px';
		}
		...
		echo '<div class="widget edge-social-icons-group-widget ' . esc_attr($class) . '">';
		...
		for ( $n = 1; $n <= 6; $n++ ) {
			...
			if ( ! empty( $instance['icon_pack'] ) ) {
				$icon_class = array( 'edge-social-icon-widget' );
			...
			<?php if ( ! empty( $icon_holder_html ) ) { ?>
				<a class="edge-social-icon-widget-holder edge-icon-has-hover" <?php echo noizzy_edge_get_inline_attr( $hover_color, 'data-hover-color' ); ?> <?php noizzy_edge_inline_style( $icon_styles ) ?>
