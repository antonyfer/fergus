...
$row_content_width = $anchor = $content_text_aligment = $marquee_text = $simple_background_color = $simple_background_image = $disable_background_image = $parallax_background_image = $parallax_bg_speed = $parallax_bg_height = $row_btt_skin = '';
...
if ( ! empty( $anchor ) ) {
	$wrapper_attributes[] = 'data-edge-anchor="' . esc_attr( $anchor ) . '"';
}
...
if ( $row_content_width !== 'grid' ) {
	...
	if (!empty($parallax_background_image)) {
		...
		$css_classes[] = 'edge-parallax-row-holder';
    ...
    if (!empty($row_btt_skin)){
        $css_classes[] = ($row_btt_skin) == 'light' ? 'edge-row-btt-light' : '';
    }
	...
	if (!empty($parallax_background_image)) {
		...
		$grid_row_class[] = 'edge-parallax-row-holder';
    ...
    if (!empty($row_btt_skin)){
        $grid_row_class[] = ($row_btt_skin) == 'light' ? 'edge-row-btt-light' : '';
    }
}
...
if($row_content_width === 'grid'){
	$edge_row_wrapper_start .= '<div class="edge-row-grid-section-wrapper '.esc_attr($grid_row_classes).'" '.esc_attr($grid_row_datas).' ' . noizzy_edge_get_inline_style( $edge_grid_row_styles ) . '><div class="edge-row-grid-section">';
...
if ( ! empty( $marquee_text ) ) {
    ...
    $output .= '<div class="edge-row-marquee">';
    $output .= noizzy_edge_execute_shortcode( 'edge_text_marquee', $params );
    $output .= '</div>';
}
