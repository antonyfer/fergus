...
$edge_rev_style = array();

if ( $enable_paspartu === 'yes' ) {
	$css_class .= ' edge-rev-has-paspartu';
	
	if ( ! empty( $paspartu_size ) ) {
		$css_class .= ' edge-paspartu-' . $paspartu_size;
	}
	
	if ( $disable_side_paspartu === 'yes' ) {
		$css_class .= ' edge-side-paspartu-disabled';
	}
	
	if ( $disable_top_paspartu === 'yes' ) {
		$css_class .= ' edge-top-paspartu-disabled';
	}
}
