...
if ( ! function_exists( 'noizzy_edge_add_custom_nav_fields' ) ) {
	function noizzy_edge_add_custom_nav_fields( $menu_item ) {
	...
	add_filter( 'wp_setup_nav_menu_item', 'noizzy_edge_add_custom_nav_fields' );
}
 ...
 * @return      void
 */
if ( ! function_exists( 'noizzy_edge_update_custom_nav_fields' ) ) {
	function noizzy_edge_update_custom_nav_fields( $menu_id, $menu_item_db_id, $args ) {
		$check = array(
			'anchor',
			'nolink',
			'hide',
		...
		if ( isset( $_POST['edge_menu_options'] ) ) {
			parse_str( urldecode( $_POST['edge_menu_options'] ), $parse_array );
	...
	// save menu custom fields
	add_action( 'wp_update_nav_menu_item', 'noizzy_edge_update_custom_nav_fields', 10, 3 );
}
 ...
 * @return      void
 */
if ( ! function_exists( 'noizzy_edge_edit_walker' ) ) {
	function noizzy_edge_edit_walker( $walker, $menu_id ) {
		return 'NoizzyEdge_Walker_Nav_Menu_Edit_Custom';
	}
	
	// edit menu walker
	add_filter( 'wp_edit_nav_menu_walker', 'noizzy_edge_edit_walker', 10, 2 );
}

if ( ! function_exists( 'noizzy_edge_include_custom_walkers' ) ) {
	function noizzy_edge_include_custom_walkers() {
		...
		do_action( 'noizzy_edge_include_custom_walkers_nav' );
	}
	
	add_action( 'after_setup_theme', 'noizzy_edge_include_custom_walkers' );
}